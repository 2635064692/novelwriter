...
export const worldModelZhMessages = {
  ...
  'worldModel.outline.title': '大纲管理',
  'worldModel.outline.description': '在这里生成卷纲、章纲，并用审核后的结果回注续写上下文。',
  'worldModel.outline.tabVolumes': '卷纲',
  'worldModel.outline.tabChapters': '章纲',
  'worldModel.outline.guidanceLabel': '追加指导',
  'worldModel.outline.guidancePlaceholder': '补充风格、节奏、卷划分偏好或关键剧情约束。',
  'worldModel.outline.totalVolumesHint': '总卷数建议',
  'worldModel.outline.batchSize': '每批章数',
  'worldModel.outline.generateVolumes': '生成卷纲',
  'worldModel.outline.generatingVolumes': '正在生成卷纲...',
  'worldModel.outline.generateChapters': '生成章纲',
  'worldModel.outline.generatingChapters': '正在生成章纲...',
  'worldModel.outline.chapterGenerationHint': '章纲生成前需先审批对应卷纲；批量较大时会自动分批输出。',
  'worldModel.outline.approveAllVolumes': '审批全部卷纲',
  'worldModel.outline.approveVolume': '审批当前卷',
  'worldModel.outline.stop': '停止',
  'worldModel.outline.activityTitle': '流式进度',
  'worldModel.outline.activityEmpty': '开始生成后，这里会实时显示进度与完成事件。',
  'worldModel.outline.summaryVolumes': ({ count }) => `共 ${String(count ?? 0)} 卷`,
  'worldModel.outline.summaryStatus': ({ status }) => `系统状态：${String(status ?? '')}`,
  'worldModel.outline.summarySelected': ({ volume }) => `当前选择：第 ${String(volume ?? '')} 卷`,
  'worldModel.outline.volumeListTitle': '卷列表',
  'worldModel.outline.empty': '暂无大纲数据，先从左侧生成卷纲。',
  'worldModel.outline.selectPrompt': '先选择一卷，或先生成卷纲。',
  'worldModel.outline.volumeFallback': ({ volume }) => `第 ${String(volume ?? '')} 卷`,
  'worldModel.outline.statusApproved': '已审批',
  'worldModel.outline.statusDraft': '草稿',
  'worldModel.outline.chapterCount': ({ count }) => `${String(count ?? 0)} 章纲`,
  'worldModel.outline.volumeRange': ({ start, end }) => `章节范围：第 ${String(start ?? '')} 章 - 第 ${String(end ?? '')} 章`,
  'worldModel.outline.volumeRangeShort': ({ start, end }) => `${String(start ?? '')} - ${String(end ?? '')} 章`,
  'worldModel.outline.volumeOutlineTitle': '卷纲内容',
  'worldModel.outline.volumeOutlineEmpty': '当前卷还没有卷纲内容。',
  'worldModel.outline.chapterListTitle': '章纲列表',
  'worldModel.outline.chapterEmpty': '当前卷还没有章纲；先生成章纲。',
  'worldModel.outline.chapterFallback': ({ chapter }) => `第 ${String(chapter ?? '')} 章`,
  'worldModel.outline.chapterBriefEmpty': '暂无章纲摘要。',
  'worldModel.outline.chapterMeta': ({ suspense, twist }) => `悬念密度：${String(suspense ?? '')}；认知颠覆：${String(twist ?? '')}`,
  'worldModel.outline.activity.startVolume': ({ count }) => `开始分析全书，共 ${String(count ?? 0)} 章，准备生成卷纲。`,
  'worldModel.outline.activity.startChapter': ({ count }) => `开始生成章纲，目标卷数：${String(count ?? 0)}。`,
  'worldModel.outline.activity.volumeReady': ({ volume, start, end }) => `第 ${String(volume ?? '')} 卷卷纲已生成（第 ${String(start ?? '')}-${String(end ?? '')} 章）。`,
  'worldModel.outline.activity.volumeStart': ({ volume }) => `开始处理第 ${String(volume ?? '')} 卷章纲。`,
  'worldModel.outline.activity.chapterReady': ({ volume, chapter }) => `第 ${String(volume ?? '')} 卷 · 第 ${String(chapter ?? '')} 章章纲已生成。`,
  'worldModel.outline.activity.batchDone': ({ volume, batch, total }) => `第 ${String(volume ?? '')} 卷已完成第 ${String(batch ?? '')}/${String(total ?? '')} 批。`,
  'worldModel.outline.activity.volumeDone': ({ volume, count }) => `第 ${String(volume ?? '')} 卷章纲完成，共 ${String(count ?? 0)} 章。`,
  'worldModel.outline.activity.doneVolume': ({ count }) => `卷纲生成完成，共 ${String(count ?? 0)} 卷。`,
  'worldModel.outline.activity.doneChapter': ({ count }) => `章纲生成完成，共 ${String(count ?? 0)} 章。`,
  'worldModel.outline.activity.aborted': '已停止当前生成流程。',
  'worldModel.outline.toast.approvedVolume': ({ volume }) => `第 ${String(volume ?? '')} 卷已审批。`,
  'worldModel.outline.toast.approvedAll': '全部卷纲已审批。',
  'worldModel.outline.error.generic': '大纲生成失败，请稍后重试。',
  'worldModel.outline.error.invalidRequest': '当前请求无法执行，请先检查卷纲审批状态或输入参数。',
  'worldModel.outline.error.approveFailed': '审批失败，请重试。',
...
export const continuationZhMessages = {
  ...
  'studio.chapter.immersive': '沉浸',
  'studio.chapter.exitImmersive': '退出沉浸',
...
export const novelEnMessages = {
  ...
  'worldModel.outline.title': 'Outline manager',
  'worldModel.outline.description': 'Generate volume and chapter outlines here, then feed approved structure back into continuation context.',
  'worldModel.outline.tabVolumes': 'Volumes',
  'worldModel.outline.tabChapters': 'Chapters',
  'worldModel.outline.guidanceLabel': 'Extra guidance',
  'worldModel.outline.guidancePlaceholder': 'Add style, pacing, volume split preferences, or key plot constraints.',
  'worldModel.outline.totalVolumesHint': 'Suggested volume count',
  'worldModel.outline.batchSize': 'Chapters per batch',
  'worldModel.outline.generateVolumes': 'Generate volume outlines',
  'worldModel.outline.generatingVolumes': 'Generating volume outlines...',
  'worldModel.outline.generateChapters': 'Generate chapter outlines',
  'worldModel.outline.generatingChapters': 'Generating chapter outlines...',
  'worldModel.outline.chapterGenerationHint': 'Approve the target volume outline before generating chapter outlines; large ranges stream in batches automatically.',
  'worldModel.outline.approveAllVolumes': 'Approve all volume outlines',
  'worldModel.outline.approveVolume': 'Approve current volume',
  'worldModel.outline.stop': 'Stop',
  'worldModel.outline.activityTitle': 'Live progress',
  'worldModel.outline.activityEmpty': 'Progress and completion events will appear here once generation starts.',
  'worldModel.outline.summaryVolumes': ({ count }) => `${String(count ?? 0)} volumes`,
  'worldModel.outline.summaryStatus': ({ status }) => `System status: ${String(status ?? '')}`,
  'worldModel.outline.summarySelected': ({ volume }) => `Selected: Volume ${String(volume ?? '')}`,
  'worldModel.outline.volumeListTitle': 'Volumes',
  'worldModel.outline.empty': 'No outline data yet. Generate volume outlines from the left panel first.',
  'worldModel.outline.selectPrompt': 'Select a volume, or generate volume outlines first.',
  'worldModel.outline.volumeFallback': ({ volume }) => `Volume ${String(volume ?? '')}`,
  'worldModel.outline.statusApproved': 'Approved',
  'worldModel.outline.statusDraft': 'Draft',
  'worldModel.outline.chapterCount': ({ count }) => `${String(count ?? 0)} chapter outlines`,
  'worldModel.outline.volumeRange': ({ start, end }) => `Chapters ${String(start ?? '')} - ${String(end ?? '')}`,
  'worldModel.outline.volumeRangeShort': ({ start, end }) => `Ch. ${String(start ?? '')} - ${String(end ?? '')}`,
  'worldModel.outline.volumeOutlineTitle': 'Volume outline',
  'worldModel.outline.volumeOutlineEmpty': 'This volume does not have outline text yet.',
  'worldModel.outline.chapterListTitle': 'Chapter outlines',
  'worldModel.outline.chapterEmpty': 'No chapter outlines yet; generate them first.',
  'worldModel.outline.chapterFallback': ({ chapter }) => `Chapter ${String(chapter ?? '')}`,
  'worldModel.outline.chapterBriefEmpty': 'No chapter brief yet.',
  'worldModel.outline.chapterMeta': ({ suspense, twist }) => `Suspense: ${String(suspense ?? '')}; twist: ${String(twist ?? '')}`,
  'worldModel.outline.activity.startVolume': ({ count }) => `Starting whole-book analysis across ${String(count ?? 0)} chapters to generate volume outlines.`,
  'worldModel.outline.activity.startChapter': ({ count }) => `Starting chapter-outline generation for ${String(count ?? 0)} volume(s).`,
  'worldModel.outline.activity.volumeReady': ({ volume, start, end }) => `Volume ${String(volume ?? '')} outline is ready (chapters ${String(start ?? '')}-${String(end ?? '')}).`,
  'worldModel.outline.activity.volumeStart': ({ volume }) => `Starting chapter outlines for volume ${String(volume ?? '')}.`,
  'worldModel.outline.activity.chapterReady': ({ volume, chapter }) => `Volume ${String(volume ?? '')} · chapter ${String(chapter ?? '')} outline is ready.`,
  'worldModel.outline.activity.batchDone': ({ volume, batch, total }) => `Volume ${String(volume ?? '')} finished batch ${String(batch ?? '')}/${String(total ?? '')}.`,
  'worldModel.outline.activity.volumeDone': ({ volume, count }) => `Volume ${String(volume ?? '')} chapter outlines finished (${String(count ?? 0)} chapters).`,
  'worldModel.outline.activity.doneVolume': ({ count }) => `Volume-outline generation finished (${String(count ?? 0)} volumes).`,
  'worldModel.outline.activity.doneChapter': ({ count }) => `Chapter-outline generation finished (${String(count ?? 0)} chapters).`,
  'worldModel.outline.activity.aborted': 'The current generation flow was stopped.',
  'worldModel.outline.toast.approvedVolume': ({ volume }) => `Volume ${String(volume ?? '')} approved.`,
  'worldModel.outline.toast.approvedAll': 'All volume outlines approved.',
  'worldModel.outline.error.generic': 'Outline generation failed. Please try again.',
  'worldModel.outline.error.invalidRequest': 'This request cannot run yet. Check approval status or request fields first.',
  'worldModel.outline.error.approveFailed': 'Approval failed. Please try again.',
  ...
  'studio.chapter.immersive': 'Immersive',
  'studio.chapter.exitImmersive': 'Exit immersive',
