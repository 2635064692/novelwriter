...
import type { CopilotMode, CopilotScope, CopilotSessionListItem } from '@/types/copilot'
import {
...
const MODE_FILTERS: { key: ModeFilter; labelKey: string }[] = [
...
function runStatusLabel(status: string | null, t: (key: string) => string): string {
  if (!status) return ''
  const key = `copilot.runStatus.${status}` as const
  const result = t(key)
...
export function CopilotSessionHistory({ novelId, onRestore, onBack }: CopilotSessionHistoryProps) {
  ...
  const loadMore = useCallback(() => {
    ...
    copilotApi.listSessions(novelId, params)
      ...
      .catch(() => {})
  }, [novelId, page, totalPages, modeFilter])
